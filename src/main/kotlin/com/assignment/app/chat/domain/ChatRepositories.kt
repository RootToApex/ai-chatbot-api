...
interface ChatThreadRepository : JpaRepository<ChatThread, Long> {
     ...
     * 유저 단위 잠금이라 서로 다른 유저의 요청은 막지 않는다.
     */
    @Query(value = "SELECT 1 FROM pg_advisory_xact_lock(:userId)", nativeQuery = true)
    fun lockUser(@Param("userId") userId: Long): Int

    /** 30분 경계 판정용 — 유저의 가장 최근 스레드 1건 */
    fun findFirstByUserIdOrderByLastQuestionAtDesc(userId: Long): ChatThread?

    fun findByUserIdOrderByCreatedAtAscIdAsc(userId: Long, pageable: Pageable): Page<ChatThread>
    fun findByUserIdOrderByCreatedAtDescIdDesc(userId: Long, pageable: Pageable): Page<ChatThread>
...
interface ChatRepository : JpaRepository<Chat, Long> {
    ...
    fun findByThreadIdInOrderByCreatedAtAscIdAsc(threadIds: Collection<Long>): List<Chat>

    /** LLM에 보낼 이력 — 최근 N개를 역순으로 가져와 호출부가 뒤집는다 */
    fun findByThreadIdOrderByCreatedAtDescIdDesc(threadId: Long, pageable: Pageable): List<Chat>
